...
9. **EL ARNÉS NUNCA PUEDE SER MÁS CÓMODO QUE LA REALIDAD.** Es el anti-patrón más caro que existe en desarrollo autónomo: la suite verde mientras la funcionalidad está rota. Las ocho formas que adopta (todas observadas en proyectos reales; ninguna teórica):
   ...
   - **EL TEST DICE CONSERVAR UNA CLÁUSULA, PERO LA EJERCITA EN OTRA CAPA.** La regla 8 del `planning.md` manda convertir en test permanente toda cláusula determinista de una Verificación. El fallo está en el «cómo»: si la Verificación dice *«`curl localhost:3000/api/health` devuelve `{ok:true}`»* y el test permanente llama al handler **en proceso**, no conserva la cláusula — conserva otra cosa, y encima **declara por escrito que la conserva**. Lección aprendida (devtools, T0.1): `route.test.ts` se presentaba como «la conservación permanente de esta cláusula, no solo un curl one-shot del verifier»; llamaba a `GET()` en proceso bajo `NODE_ENV=test`, donde la rama `pretty:true` del logger no se construye. `pino-pretty` no estaba declarado como dependencia ⇒ **HTTP 500 en TODA ruta bajo `next dev`**, con la suite en VERDE y el test que decía protegerlo también en verde. `next start` funcionaba: el fallo era **dev-only**, y dev era justo la capa que la Verificación nombraba. *La regla: **el test permanente tiene que ejercitar la MISMA capa que la Verificación** (HTTP contra el server levantado si la cláusula habla de HTTP; el bundler si habla del bundler). Si no puedes ejercitar esa capa en el gate, dilo en la tarea y deja la cláusula como one-shot del verifier — pero **no escribas un test que afirme conservar lo que no conserva**: cobertura falsa es peor que ninguna.* Corolario: **una rama de config que ningún test instancia no existe hasta que un usuario la pisa** — si `NODE_ENV`/env selecciona ramas, algún test tiene que construir cada rama de verdad.

   - **EL ASSERT NEGATIVO QUE NO PUEDE CASAR (el centinela fantasma).** Un `not.toContain(X)` solo prueba algo si `X` PODRÍA aparecer por el canal que se observa; si no puede, el assert pasa siempre — con la fuga delante. Tres mecanismos observados en devtools, todos con la suite en verde: (1) **el literal no sobrevive a las transformaciones del canal** — T2.3: el preview se trunca a 120 chars y el segmento de payload del JWT mide más, así que el segmento ENTERO no casa nunca (hay que asertar prefijos que sobrevivan al truncado); y `"sub"` en HTML se renderiza `&quot;sub&quot;`, así que el literal con comillas jamás casaría. (2) **el canal observado está VACÍO** — T0.1: una captura de logs que capturaba 0 líneas hacía pasar vacuamente todo `not.toContain`; el guard es un assert POSITIVO de que el canal lleva datos (`lines.length > 0`, o un control positivo: un literal que SÍ debe aparecer, aparece). (3) **se asserta la propiedad cómoda y no la peligrosa** — T2.2: los tests de paginación asertaban ausencia de SOLAPE pero nunca ausencia de HUECO, y el hueco era el bug. Además, si la fuga puede mutar antes de persistirse (p. ej. un input que se DECODIFICA), el grep debe cubrir también las formas derivadas — un grep del base64 no ve el JSON decodificado que sí se guardó. *La regla: **todo assert negativo de ausencia (privacidad, secretos, fuga de datos) exige en la misma pasada (a) un control positivo que pruebe que el canal observado lleva datos y el patrón apunta bien, y (b) un control negativo EJECUTADO: reinyecta la fuga y comprueba que el assert se pone ROJO.** Un assert de ausencia sin las dos patas es decoración.*

   **Las ocho preguntas, antes de dar por bueno un test:** (a) *¿Este doble emite lo que emite el productor real?* → **ve a leer el productor**, no lo supongas. (b) *¿El arnés está fijando algo que en producción se calcula?* → si sí, el test no prueba el cálculo. (c) *¿Estoy midiendo en el entorno real o en uno idealizado?* (d) *¿Estoy ejerciendo el camino que recorre el dato de verdad, o uno paralelo que he construido en el test?* (e) *¿El test corre en el MISMO runtime que producción?* → si el código viaja a un navegador, un bundler o una VPS, Node en tu portátil no es ese sitio. (f) *Si lo que pruebo es una LEY, ¿la estoy midiendo en un punto donde una ley equivocada daría lo mismo?* → entonces no la estás midiendo. (g) *Si este test dice conservar una cláusula de una Verificación, ¿la ejercita en la MISMA capa que ella?* → si la cláusula habla de HTTP y el test llama al handler en proceso, no la conserva; y si además hay ramas que dependen de env (`NODE_ENV`), ¿las instancia alguien de verdad? (h) *Si el assert es NEGATIVO (ausencia de X), ¿puede X aparecer por este canal?* → comprueba que el literal sobrevive a truncados/escapes/decodificaciones del camino real, y que el canal no está vacío — con un control positivo en la misma pasada.
   Y la prueba de fuego, que es barata y no es opcional en un fix de bug **ni en ningún assert negativo de privacidad/seguridad**: **CONTROL NEGATIVO — reintroduce el bug (o la fuga) y comprueba que el test se pone ROJO.** Un test que no has visto fallar no sabes si muerde. **Y mira QUÉ se pone rojo**: si el rojo aparece en un test paralelo mientras el validador de producción sigue diciendo `ok: true`, el rojo está en el sitio equivocado — solo mirarlo lo revela. *(No existe un mecanismo de código único que cace estas formas: viven en capas distintas y cada guard puntual es frágil por su cuenta. La defensa es este principio, aplicado por el `implementer` al escribir y por los pases de review y el `verifier` al cerrar.)*
