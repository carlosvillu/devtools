...
> **Gotcha del entorno (el bucle corre DENTRO del VPS, Ubuntu 24.04): Chrome necesita `--no-sandbox`.** Ubuntu 23.10+ trae `kernel.apparmor_restrict_unprivileged_userns = 1`, que desactiva los user namespaces sin privilegios; el sandbox de Chrome no arranca y `agent-browser doctor` da `No usable sandbox … Chrome exited before providing DevTools URL`. Es el caso estándar de contenedor/VM. Pasa el flag al abrir: `agent-browser open --args "--no-sandbox" <url>` (el resto de comandos operan sobre la sesión ya abierta, no repiten el flag). El `doctor` seguirá marcando **1 fail en «Launch test»** porque su prueba interna no acepta args — no es un bloqueo: si `open --args "--no-sandbox"` navega y `get title` lee la página, CUA está operativo. Las **librerías de sistema** del navegador (`libatk-1.0.so.0`, `libgbm.so.1`, `libcups.so.2`… en Ubuntu: `libatk1.0-0t64`, `libgbm1`, `libcups2t64`, etc.) las instala `sudo agent-browser install --with-deps` **una sola vez** — requiere root, así que lo ejecuta el usuario, no el bucle.
