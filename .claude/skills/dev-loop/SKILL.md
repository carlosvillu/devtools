...
- **EL ARNÉS NUNCA PUEDE SER MÁS CÓMODO QUE LA REALIDAD.** Es el anti-patrón más caro conocido de este tipo de bucle: la suite VERDE mientras la funcionalidad está rota. El detalle y las preguntas de control están en la skill `testing` (fuente de verdad); aquí, lo que el revisor tiene que hacer:
  ...
  - **Un test dice CONSERVAR una cláusula de la Verificación, pero la ejercita en OTRA CAPA** → cuando revises el test permanente que la regla 8 del planning exige, compara **la capa que nombra la cláusula con la capa que toca el test**. Si la Verificación dice «curl contra el servidor levantado» y el test llama al handler en proceso, **no la conserva** — y el test suele *declarar por escrito* que sí, que es lo que apaga la alarma de todos. Corolario: **una rama de config que ningún test instancia no existe** (una rama por `NODE_ENV` que solo vive en dev no la ve una suite que corre en `test`).
  ...
  - **Un assert NEGATIVO que no puede casar** → cuando revises un `not.toContain`/grep de ausencia (privacidad, secretos, fugas), pregunta si el literal PUEDE aparecer por ese canal: ¿sobrevive al truncado, al HTML-escaping, a la decodificación del camino real? ¿el canal lleva datos (control positivo en la misma pasada)? ¿cubre también las formas DERIVADAS del dato (el valor decodificado, no solo el codificado)? Nació de T2.3: asserts de privacidad estructuralmente incapaces de fallar (segmento más largo que el truncado; `"sub"` que en HTML es `&quot;sub&quot;`), verdes con la fuga delante. El detalle vive en la forma 8 de la skill `testing`.

  **CONTROL NEGATIVO — exígelo en todo fix de bug con test nuevo Y en todo assert negativo de privacidad/seguridad** (es barato y no es opcional): **reintroduce el bug (o la fuga) y comprueba que el test se pone ROJO.** Un test que nadie ha visto fallar no se sabe si muerde. **Si el implementer no lo aporta, pídeselo antes de VERIFY.**
...
- **Se salta también si `docs/design-system/` está vacío** (antes de que TD.1 lo genere): el pase revisa el diff CONTRA el espejo del DS, así que sin espejo no hay vara de medir y solo puede producir ruido. Es la única excepción a «tocó `apps/web/**` ⇒ corre», nació de T0.1 (que crea una página raíz deliberadamente sin estilos, antes de que exista el DS), y **se anota en el journal cuando se aplica** — saltárselo en silencio sí sería deriva.
...
## Premisas: se RE-MIDEN, no se recuerdan

Los incidentes más caros del bucle no fueron bugs de código sino **premisas heredadas tratadas como hechos**. Cuatro reales de este proyecto: un claim sobre pino que era falso (T0.1); un gotcha del planning (`--shadow-*` circular) que era falso y solo cayó cuando el implementer lo PROBÓ con un build real (TD.1); la premisa «prod tiene 0 usuarios» que caducó a las 09:52 y se siguió usando horas para justificar riesgos; y el único incidente de producción — se sobrescribió el `.env` de la raíz creyendo que era config de dev caducada, cuando el propio journal decía que era el de PRODUCCIÓN. Tres reglas vinculantes:

1. **Toda afirmación heredada lleva etiqueta.** Un diagnóstico de otro agente, un gotcha del planning/plantilla o una nota del journal se traslada como `verificado` (con la evidencia) o `no verificado` — y lo no verificado **se prueba antes de obedecerlo o de actuar sobre él**. Una advertencia heredada no es un hecho verificado; el implementer que la prueba en vez de obedecerla o ignorarla está haciendo lo correcto.
2. **Las premisas sobre estado externo caducan.** El estado de producción (usuarios, datos, qué SHA corre), del VPS o de cualquier sistema vivo se **re-mide en el momento de uso**, no se arrastra de una entrada de journal ni del contexto de hace horas. Especialmente antes de justificar un riesgo con él («no hay datos que filtrar») o de una acción que muta.
3. **Antes de sobrescribir o borrar config/entorno, confirma qué ES ese fichero** — no qué crees que es: grep del journal y de `deploy.env`/compose (¿quién lo consume?), y si contiene un secreto fuerte donde esperabas un literal `*-not-a-secret`, PARA: esa discrepancia es evidencia de que tu premisa está mal, no de que el fichero esté mal.

Corolario de arnés: cuando una de estas reglas se aplique de memoria dos veces, conviértela en **check mecánico** (guard en script, fail-fast, nombre que no colisiona) — como la guarda de árbol limpio de `redeploy.sh`. Una regla que vive solo en la memoria del agente volverá a fallar.
