...
| **F6** · Componer            | La pantalla de trabajo tiene dos direcciones: `/analyze` decodifica (hoy) y `/compose` **compone** — escribes un valor, encadenas transformaciones que TÚ eliges (`json.minify`, `base64.encode`, `url.encode`, `hash.sha256`, `jwt.sign`…) y copias el resultado listo para compartir. **El motor de composición corre en el navegador**: ni el texto fuente ni el secreto de firma salen de la máquina. Con cuenta, el historial guarda la **receta** (los pasos), nunca los valores | 🔨 9/11     |
