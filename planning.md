...
- **Subtareas**:
  ...
  - [ ] **Decisión pendiente que el mockup no puede resolver** (detectada en T6.4): el artboard pinta un **chip de kind por cada item de la paleta** (`base64.encode` → chip `base64`), pero el catálogo de `core` **no puede darlo** — por **I10** el kind se DETECTA sobre la salida, y cuando se pinta la paleta esa salida todavía no existe. Decide explícitamente: tabla de presentación en la UI (kind *esperado*, solo decorativo) o **quitar el chip**. No lo dejes a medias: un chip que promete un tipo que el motor podría no detectar es peor que no tenerlo.
