...
- **Entrega**: `docker-compose.prod.yml` (web Next standalone + postgres con volumen persistente; **sin worker**, §5.2), `DEPLOY.md`, deploy por `git pull && docker compose up -d --build` **vía la skill `deploy`** (autodetecta modo local: el bucle corre EN el VPS). Todo según la topología real de §10 y el `~/AGENTS.md` del VPS:
  - **La web publica solo en `127.0.0.1:3110`**, nunca en `0.0.0.0` — un puerto abierto por Docker se salta UFW. Bloque de puertos de devtools: 3110–3119.
  ...
  - **Deuda conocida del arnés** (anotada en el journal y en `deploy.env`): `redeploy.sh` intenta `docker network create edge` + `docker network connect edge edge-caddy`; en este VPS `edge-caddy` corre con `network_mode: host` y esa rama es **inerte** (falla con `warn`, no rompe el deploy). No se toca la skill: se documenta y se usa la vía loopback, que su propia SKILL.md contempla.
