...
> - **El COPY de privacidad del artboard es FALSO para nuestra implementación y NO se copia** (detectado en T6.2, 2026-07-22). El `Callout` y el aviso del panel de firma del mockup dicen que el secreto «viaja al servidor solo para firmar»: eso describe un producto que **decidimos no construir** (decisión 1 de esta fase — motor en el cliente, sin `/api/compose`). Reproducirlo sería publicar una promesa de privacidad **peor que la real** y, peor aún, **falsa**. El layout del aviso es vinculante; su texto lo reescribe **T6.8**, que es donde el copy es Entrega y no decoración.
...
#### T6.2 · Espejo: `Segmented` en el DS y el mockup `compose.html` [x] 2026-07-22 — PASS (tras una re-verificación), ver docs/verifications/T6.2/ (coste $0)
- **Depende de**: T6.1
...
#### T6.3 · La primitiva `Segmented` en código + showcase [x] 2026-07-22 — PASS (tras un FAIL), ver docs/verifications/T6.3/ (coste $0)
- **Depende de**: T6.2
...
- **Deuda heredada que esta tarea recoge** (destapada al regenerar el espejo en T6.2, 2026-07-22): el `_adherence.oxlintrc.json` de upstream había ganado, sin que el espejo se enterara, las entradas de `Dialog`, `Image`, `Wordmark` y `SegmentedOption`, los grupos `components/brand/**` y `components/overlay/**` de `no-restricted-imports`, y los **4 tokens de accesibilidad de TD.6** (`--danger-hover`, `--violet-subtle-fg`, `--cyan-subtle-fg`, `--gray-450`). Nuestro lint de adherencia (flat config de **TD.6**, adaptado a mano de ese fichero) puede estar ciego a todo eso. **Revísalo aquí**: compara el `_adherence.oxlintrc.json` ya actualizado con las reglas de `eslint.config.ts` y, si faltan reglas que el DS sí exige, **añádelas en esta tarea** (es el sitio natural: estás tocando la adherencia al DS y estrenando una primitiva). Si decides que alguna no aplica al flat config, escribe por qué — no la omitas en silencio.
...
#### T6.4 · Transformaciones de codificación sin secreto (`packages/core`) [x] 2026-07-22 — PASS, ver docs/verifications/T6.4/ (coste $0)
- **Depende de**: T6.1
...
- **Subtareas**:
  - [x] Catálogo de codificación (id, label español, grupo del mockup) como dato en `core`, **no** en la capa de presentación — misma lección que `KINDS_COEXISTING_WITH_TEXT` en I8
  - [x] Las 4 transformaciones nuevas + reutilización explícita de `json.minify`
  - [x] **Sin `node:*` ni Web Crypto ni `Buffer`**: el código corre en el navegador (D10/§5.3). Control de lint o test que lo impida, al estilo del grep de `Date.now()` de T1.2
...
#### T6.5 · Hashes puros y `jwt.sign` (HS256) — sin salir del navegador [x] 2026-07-22 — PASS, ver docs/verifications/T6.5/ (coste $0)
- **Depende de**: T6.4
...
#### T6.6 · `compose()` — el motor que dirige el usuario [x] 2026-07-22 — PASS, ver docs/verifications/T6.6/ (coste ~$1,4 · ver nota de gasto en el journal)
- **Depende de**: T6.5
...
- 🔴 **Deuda heredada BLOQUEANTE, destapada en T6.4 (2026-07-22)**: `packages/core/src/engine/detectors.ts` (líneas ~122, 150, 152) y `transforms.ts` (~130) usan **`Buffer`**, que **no existe en el navegador**. Hoy es inocuo porque el motor solo corre en el servidor; deja de serlo aquí: por **I10**, `compose()` **re-ejecuta los detectores de §6.2** sobre la salida de cada paso, y por D10 eso pasa en el cliente → **`ReferenceError` en tiempo de ejecución, no un problema latente**. Además, reutilizar `json.minify` arrastra `transforms.ts` al cono de módulos de composición. **Resuélvelo en esta tarea** (base64 puro para `detectBase64`/`decodeSegmentJson`/`base64.decode`, o una frontera de módulos explícita) y **amplía el guard `COMPOSE_MODULES` de `client-only.test.ts`** a todo el cono real. Ojo con la trampa que el propio T6.4 anotó: ese guard grepea **texto de fichero, no el grafo de imports**, así que verlo verde **no** prueba que el cono esté limpio — hazlo cubrir lo que `compose()` importa de verdad.
...
#### T6.7 · La pantalla: `/compose` y el conmutador de dirección [x] 2026-07-23 — PASS, ver docs/verifications/T6.7/ (coste $0)
...
- **Subtareas**:
  - [x] Extraer el marco compartido de la pantalla de trabajo (cabecera + `Segmented` + contenedor) sin tocar el comportamiento de `/analyze`
  - [x] `apps/web/src/app/compose/page.tsx` + la isla cliente del constructor de cadena
  - [x] Reproducir el layout del mockup **con primitivas del DS** (`Textarea`, `Select`, `Field`, `Input`, `IconButton`, `Badge`, `CopyButton`, `Card`, `Callout`, `Icon`, `Segmented`) — nada de marcado crudo estilado (`ds-reviewer`)
  - [x] Responsive contra `ComposeClaroM`: los pasos se apilan, la paleta no desborda, sin scroll horizontal del body
  - [x] **Decisión pendiente que el mockup no puede resolver** (detectada en T6.4): el artboard pinta un **chip de kind por cada item de la paleta** (`base64.encode` → chip `base64`), pero el catálogo de `core` **no puede darlo** — por **I10** el kind se DETECTA sobre la salida, y cuando se pinta la paleta esa salida todavía no existe. Decide explícitamente: tabla de presentación en la UI (kind *esperado*, solo decorativo) o **quitar el chip**. No lo dejes a medias: un chip que promete un tipo que el motor podría no detectar es peor que no tenerlo. → **Resuelto: se quita el chip** (`compose-builder.tsx`), con las razones escritas junto a la paleta.
- **Lo que T6.6 te dejó medido y avisado** (2026-07-22, para que no lo redescubras en la verificación visual):
  - **Usa `safeCompose`, no `compose`**: `compose()` **lanza** con una receta malformada (p. ej. un paso con `transform: ''` mientras el usuario aún no ha elegido en el `Select` — y tú recompones en cada pulsación, así que ese estado transitorio ES camino caliente, no un borde). `safeCompose` devuelve el fallo como dato.
  - **La barra de resultado se gatea con `terminal === 'ok'` y `steps.length > 0`, NO con `output != null`**: con `terminal:'error'`, `output` es el **parcial del último paso correcto**, así que cablearla mal hace que el usuario **copie un resultado incompleto** bajo una pantalla que muestra un fallo. Y con receta vacía `output === source`, así que sin el segundo gate la barra duplica la fuente bajo un «· 0 pasos ·». El contador del artboard debe ser `steps.filter(s => s.ok).length`.
  - **Con el campo vacío, `sourceKind` es `text`** (el suelo de I6), así que hay que suprimir el `Badge` de «reconocido» a mano: el motor no distingue «vacío» de «texto».
  - **Presupuesto medido**: `compose()` con 2 pasos cuesta **207 µs** sobre 42 bytes y **13,7 ms** sobre 128 KB (el tope de I7). Recomputar la receta entera en cada cambio es viable — **no** hace falta API incremental ni worker.
...
- **Entrega**: con sesión iniciada, componer deja rastro en `/history` **sin que un solo carácter del usuario salga del navegador**:
  ...
  - 🔴 **OBLIGACIÓN DE SEGURIDAD heredada de T6.5 (2026-07-22), la más fácil de que se cuele**: el secreto de firma vive en **`ComposeStepSpec.options`**, y §9 dice que **la receta es lo que se persiste**. `jwt.sign` mantiene el secreto fuera de su `TransformResult`, pero **nada en `core` puede impedir que este borde escriba `options.secret` en la BD**: es exactamente el camino por el que un secreto acabaría en `psql`, que es lo que toda la fase promete que no pasa. La frontera de persistencia **excluye `options` de forma explícita** (allowlist de campos, no blacklist) y lo prueba con **su propio canario**: firmar con `test-signing-secret-not-a-secret`, `pg_dump --data-only` de la BD completa, `grep` → 0 coincidencias, con control positivo en la misma pasada.
