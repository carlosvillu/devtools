...
// ── el motor de COMPOSICIÓN (§6.6, T6.6) ────────────────────────────────────────────

// Lo que el usuario ELIGE: la receta. Es lo único que se persiste (§9) — y por eso `options`
// es un `Record` abierto y no un objeto tipado por transformación: quien decide qué opciones
// acepta un paso es el cuerpo de la transformación (el descriptor `EncodeOptionSpec` solo
// describe el formulario). El secreto de `jwt.sign` viaja aquí y NO en el resultado; excluirlo
// antes de escribir es trabajo de la frontera de persistencia (T6.10), no del motor.
export const ComposeStepSpecSchema = z.object({
  transform: z.string().min(1), // Transform.id del catálogo de CODIFICACIÓN (§6.6)
  options: z.record(z.string(), z.unknown()).optional(),
});
export type ComposeStepSpec = z.infer<typeof ComposeStepSpecSchema>;

// Límite de 8 pasos (§6.6, el mismo de I2). Vive en el ESQUEMA, no en el cuerpo de `compose()`:
// el PRD es explícito («una receta de 9 pasos no se ejecuta a medias, se rechaza»), y así la UI
// no puede construir una cadena que el motor rechace — valida la receta con este esquema antes
// de ofrecer el botón. `compose()` lo aplica en el borde: una receta de 9 pasos LANZA (no
// devuelve un resultado a medias), lo que no contradice I9, que gobierna la EJECUCIÓN de los
// pasos. Una receta vacía es válida (0 ≤ 8) y da `output === source` (I12).
export const MAX_COMPOSE_STEPS = 8;
export const ComposeRecipeSchema = z.array(ComposeStepSpecSchema).max(MAX_COMPOSE_STEPS);
export type ComposeRecipe = z.infer<typeof ComposeRecipeSchema>;

// Lo que el motor DEVUELVE por cada paso ejecutado (§6.6), tal y como lo consume la UI.
// `index` empieza en 1: la fuente es el paso 0 y no es un `ComposeStep` (asimetría deliberada
// con `ChainStep`, cuyo paso 0 SÍ es un paso porque allí la fuente ya se detecta y transforma).
// `kind` es `DataKind | null` porque se DETECTA sobre la salida (I10) y un paso fallido no
// tiene salida que detectar. NO existe un campo `options`: el secreto de `jwt.sign` no puede
// llegar al resultado (§11), y la forma más barata de garantizarlo es que no haya dónde ponerlo.
export const ComposeStepSchema = z.object({
  index: z.number().int().positive(),
  transform: z.string(),
  input: z.string(),
  ok: z.boolean(),
  output: z.string().nullable(), // null si ok === false
  kind: DataKindSchema.nullable(), // DETECTADO sobre la salida (I10), nunca declarado
  error: z.string().optional(), // presente si ok === false
  notes: z.array(z.string()).optional(), // del TransformResult (§6.1)
});
export type ComposeStep = z.infer<typeof ComposeStepSchema>;

// Solo dos finales, frente a los cinco de `ChainTerminal`: `compose()` no se auto-conduce (I12),
// así que no existen `no_transform`, `max_depth` ni `cycle` — no hay bucle que cortar.
export const COMPOSE_TERMINALS = ['ok', 'error'] as const;
export const ComposeTerminalSchema = z.enum(COMPOSE_TERMINALS);
export type ComposeTerminal = z.infer<typeof ComposeTerminalSchema>;

// `output` es la salida del ÚLTIMO PASO OK (§6.6), no la del último paso: si un paso falla, lo
// anterior se conserva (I9). Es `null` solo cuando no hay ningún paso ok y la receta no estaba
// vacía (es decir, cuando falló el primer paso); con receta vacía es `source` (I12).
//
// ██ DOS TRAMPAS DE CABLEADO PARA LA UI (T6.7) ██ Las dos nacen de que este contrato prioriza
// «no perder el trabajo del usuario» (I9) sobre «tener un único campo que signifique éxito».
// Se escriben aquí, en el contrato, porque son bugs de pantalla plausibles y el sitio barato de
// evitarlos es antes de escribir la pantalla, no en su verificación visual:
//
//   1. **`output != null` NO significa «salió bien».** Con `terminal:'error'`, `output` y
//      `outputKind` son el PARCIAL del último paso ok. Una barra de resultado cableada a
//      `output != null` dejaría al usuario copiar un resultado incompleto mientras la pantalla
//      le dice que algo falló. La condición de éxito es **`terminal === 'ok'`**.
//      Por lo mismo, el «· N pasos ·» del artboard debe contar `steps.filter(s => s.ok).length`
//      y no `steps.length`: el array incluye el paso que falló.
//   2. **Con receta vacía, `output === source` y `outputKind === sourceKind`** (I12: la fuente
//      tal cual). Una barra de resultado que solo mire `output` DUPLICARÍA la fuente en
//      pantalla bajo un «· 0 pasos ·». Hay que gatearla también en `steps.length > 0`.
export const ComposeResultSchema = z.object({
  source: z.string(),
  sourceKind: DataKindSchema, // detectado sobre la fuente con los detectores de §6.2
  steps: z.array(ComposeStepSchema), // solo los ejecutados; se corta en el primero que falla
  output: z.string().nullable(),
  outputKind: DataKindSchema.nullable(),
  terminal: ComposeTerminalSchema,
});
