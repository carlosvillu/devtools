...
function isJwtHeaderSegment(segment: string): boolean {
  ...
  // su decodificación + `JSON.parse`: ~900 ms de event loop bloqueado. Es cota, no
  // adivinanza, así que no puede divergir de `decodeJwtHeader` (ver más abajo).
  // (Cifra RE-MEDIDA en T6.6, que sustituyó el `Buffer.from` de `decodeSegmentJson` por el
  // decodificador puro de `engine/base64.ts` para que el motor corra en el navegador. En la
  // misma máquina: ~750 ms con `Buffer`, ~900 ms ahora; la anterior anotación decía 1.061 ms,
  // medida en otra máquina. Lo que justifica la cota es el ORDEN DE MAGNITUD, que no ha
  // cambiado — pero un número obsoleto en un comentario es deuda, así que se actualiza.)
